...
void Behavior()
{
  ...
  switch(currentMode)
  {
    ...
    case 1: //slow to fast
      startColourSpeed = 3000;
      endColourSpeed   = 300;
      maxTimeToFallASleep = 150;
      break;
    case 2:
      startColourSpeed = 300;
      endColourSpeed   = 3000;
      maxTimeToFallASleep = 1500;
      break;
  }
  ...
  case 1: //Sleeping
    ...
    /*case 2:
     ...
     break;*/
  case 3: // Beeing touched
    {
      if(capaValue > capaValueState1) // The box is beeing touched
      {
        int MIN_TOUCH_LENGTH =  2000;
        int MAX_TOUCH_LENGTH = 10000;
        state = 3;
        lastTimeTouched = millis();
        touchLength = lastTimeTouched - touchStarted;
        
        if (touchLength < MIN_TOUCH_LENGTH) {
          COLOUR_SPEED = startColourSpeed;
        }
        else if (touchLength < MAX_TOUCH_LENGTH) {
          COLOUR_SPEED = round(map(touchLength, MIN_TOUCH_LENGTH, MAX_TOUCH_LENGTH, startColourSpeed, endColourSpeed));
        }
        else {
          COLOUR_SPEED = endColourSpeed;
        }
        //Serial.println(COLOUR_SPEED);
      }
...
void Behavior3() //The box is being touched
{
  lightBrightness = 255;
  cycleColours();
  byte motorSpeed = map(COLOUR_SPEED, 0, 3000, 255, 1);
