...
void showState()
{
  ...
  if (use_anger) {
	...
	if (use_tilt){
		dp = towardsZero(dp,1);
		dr = towardsZero(dr,1);
		lr = constrain(abs(dp)*2.5, 0, 255);
		lg = constrain(abs(dr)*2.5, 0, 255);
		lb = 0;
		sp = 25;
	}
	  else {
		...
		dy = towardsZero(dy,1);
		dz = towardsZero(dz,1);
		lr = constrain(abs(dx)*2.5, 0, 255);
		lg = constrain(abs(dy)*2.5, 0, 255);
		lb = constrain(abs(dz)*2.5, 0, 255);
		sp = 25;
	}
  }
  //pummer.loop();
