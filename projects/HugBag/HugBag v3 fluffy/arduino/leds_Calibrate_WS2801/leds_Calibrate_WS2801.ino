...
void HandleControlChange(byte channel, byte number, byte value) {
  ...
  switch (number) {
    ...
    case 8: // Total Volume
      val = map(value, 0, 127, 64, 255) * FILTERSPEED + val * (1-FILTERSPEED);
    case 4: // Brus Volume
      color[0] = (float)value*(float)val/127 * FILTERSPEED + color[0] * (1-FILTERSPEED);
      break;
    case 5: // Pling. Volume
      //color[1] = (float)value*(float)val/127 * FILTERSPEED + color[1] * (1-FILTERSPEED);
      break;
    case 6: // Pitchtone Volume
      color[2] = (float)value*(float)val/127 * FILTERSPEED + color[2] * (1-FILTERSPEED);
      break;
...
void encoder() {
  ...
  if (tmpdata != rot_counter) {
    ...
    rot_counter = tmpdata;
    slow_counter = constrain(rot_counter/2, 0, 100);
    for (int i = 0; i < nLEDs; i++) {
      if (slow_counter == i)
        mystrip[i][1] = 200;
      else
