...
gRecoilType <- regenum("Recoil", "Recoil",
			   ...
			   regvalue("LoopRecoil", KEY_3, "Movement Recoil Effect"));
...
gKeyBinds.moveRecoil <- gRecoilType.value(3);
...
// create a simple recoil effect and set its parameters.
gMoveRecoil <- effectparameters(gEnvelopeEffectID, gEffectsStack);	// create the effect
gMoveRecoil.setvarelement("force", 0, 0); // 0 newtons right-wards
gMoveRecoil.setvarelement("force", 1, 0);	// 0 newtons upwards
gMoveRecoil.setvarelement("force", 2, 0); // 0 newtons backwards
gMoveRecoil.setvar("attack", 30); // ramp up to the force over 30 milliseconds.
gMoveRecoil.setvar("hold", 0); // no hold time once at maximum force
gMoveRecoil.setvar("decay", 0); // no fall off time at end of effect

// variable to store which type of recoil effect to use
gEffectType <- 3;	// 0 = no effect, 1 = simple recoil, 2 = looping recoil... init to 1
...
function HapticsThink (deviceHandle)
{
	...
	// get the position of the Falcon
	...
	else if (gKeyInput.wasinputjustpressed(gKeyBinds.moveRecoil))
	{
		print("   ... select movement recoil\n");

		// set effect type to movement recoil
		gEffectType = 3;
	}
	...
	//local moved_by_keys = false;
	//if (moveX != 0 || moveY != 0 || moveZ != 0) {
	//	moved_by_keys = true;
	//}
	
	// get the velocity of the push and then move the falcon away
	local vel = gControlBox.getvelocity();
	local vel_thres = 0.15;
	local vel_scale = 5;
	
	//Throttle Up Control
	if (gEffectType == 3) {
		if(fabs(vel.x) > vel_thres || fabs(vel.y) > vel_thres || fabs(vel.z) > vel_thres)
		{
			// count down to next time to fire off a recoil
			/*gLoopRecoilTimer -= elapsedTime;	// subtract elasped time from the timer
			if (gLoopRecoilTimer <= 0.0)
			...
			*/
			if (falcon_X > 0)
				gMoveRecoil.setvarelement("force", 0, -vel_scale*fabs(vel.y));
			else
				gMoveRecoil.setvarelement("force", 0, vel_scale*fabs(vel.y));
			if (falcon_Y > 0)
				gMoveRecoil.setvarelement("force", 1, -vel_scale*fabs(vel.x));
			else
				gMoveRecoil.setvarelement("force", 1, vel_scale*fabs(vel.x));
				
			gMoveRecoil.setvarelement("force", 2, -vel_scale*vel.z);
			
			gMoveRecoil.fire();
				
			/*	// reset timer... fire off a recoil every tenth of a second
				gLoopRecoilTimer += 0.3;	// increase timer by a tenth of a second
			}*/
		}
		//gMovementForce.setforce(-5*vel.y, -5*vel.x, 5*vel.x + 5*vel.y);
	}
	else {
